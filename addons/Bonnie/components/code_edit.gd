...
var theme_overrides: Dictionary:
	set(value):
		...
		# Conditions
		syntax_highlighter.add_keyword_color("shuffle", theme_overrides.conditions_color)
		syntax_highlighter.add_keyword_color("cycle", theme_overrides.conditions_color)
		syntax_highlighter.add_keyword_color("once", theme_overrides.conditions_color)
		syntax_highlighter.add_keyword_color("sequence", theme_overrides.conditions_color)
		syntax_highlighter.add_keyword_color("when", theme_overrides.conditions_color)
		...
		syntax_highlighter.add_keyword_color("trigger", theme_overrides.conditions_color)
		syntax_highlighter.add_keyword_color("set", theme_overrides.conditions_color)
		...
		syntax_highlighter.add_color_region("[{"," ", theme_overrides.logic_color)
		syntax_highlighter.add_color_region("}]"," ", theme_overrides.logic_color)
		syntax_highlighter.add_color_region("{"," ", theme_overrides.logic_color)
		syntax_highlighter.add_color_region("}", " ", theme_overrides.logic_color)
		syntax_highlighter.add_color_region("[", "]",theme_overrides.logic_color)
		...
		# Jumps
		syntax_highlighter.add_color_region("->"," ", theme_overrides.jumps_color)
		syntax_highlighter.add_color_region("<-"," ", theme_overrides.jumps_color)
...
func _can_drop_data(at_position: Vector2, data) -> bool:
	...
	var files: PackedStringArray = Array(data.files).filter(func(f): return f.get_extension() == "clyde")
	return files.size() > 0
...
func _drop_data(at_position: Vector2, data) -> void:
	...
	for file in files:
		...
		var path = file.replace("res://", "").replace(".bonnie", "")
...
func _request_code_completion(force: bool) -> void:
	...
	if ("=> " in current_line or "=>< " in current_line) and (cursor.x > current_line.find("=>")):
		...
		var parser: BonnieParser = BonnieParser.new()
...
func insert_text(text: String, line: int, column: int, before_selection_begin: bool = true, before_selection_end: bool = false):
	if text != "=> END":
...
func _on_code_edit_symbol_validate(symbol: String) -> void:
	if symbol.begins_with("res://") and symbol.ends_with(".bonnie"):
